...
|---|---|---|---|---|---|
| `add.paper` | `paper` | `paper` | Phase 2A internal foundation; Phase 2B resolver | DOI/arXiv exact candidate search、itemType classification、Paper metadata and attachment recovery | `Implemented` |
| `add.web` | `web` | `web` | Phase 2B | URL canonicalization、exact Zotero webpage and immutable HTML/XHTML snapshot evidence | `Implemented` |
| `add.book` | `book` | `book` | Phase 2B | DOI/ISBN metadata、edition identity、Zotero mapping | `Implemented` |
| `add.repo` | `repo` | `oss` | Phase 2B | configured Git host、isolated anonymous remote HEAD resolution、immutable commit、`license: NOASSERTION`；整体项目笔记复用 Literature Note | `Implemented` |
